...
## Phase 3 — Race Mode (Time Attack)

**Status: done.** `RaceGame` (`backend/src/domain/entities/race-game.ts`) and `RaceMode` (`backend/src/application/game-modes/race-mode.ts`) implement the rules below; `WordRound` (Phase 2) is reused unmodified (`maxAttempts: Infinity`) for per-player infinite-attempt rounds — no changes to Phase 2 domain code. 15 backend tests pass — `race-game.spec.ts` covers independent per-player word progression, the must-solve-all-5-to-win rule, the timeout-disqualifies-but-doesn't-block-you rule, and the no-winner case; `race-mode.spec.ts` adds per-player timer-expiry (via `vi.useFakeTimers`) and confirms a resolved player's timeout never fires after they solve early. Frontend (`frontend/src/hooks/useRaceGame.ts`, `frontend/src/components/{RaceStatus,RaceLeaderboard,RaceSummary}.tsx`) adds a per-player word-index/timer indicator, a race leaderboard (progress through the 5 words, not score), and a race-finished screen; both packages typecheck and build clean.

**Deviation from Championship Mode's UI (deliberate, requested post-launch)**: Race Mode's live screen deliberately shows less than Championship Mode's — no attempts counter and no mid-race banner when a word is solved, timed out, or rejected as invalid (`RaceStatus`/`App.tsx` no longer render those; the `guess:error`/`room:error` listeners were dropped from `useRaceGame.ts` since nothing consumed them anymore). Every word resolution is still tracked (`useRaceGame`'s `revealHistory`), but only surfaced once via `RaceSummary` after the race ends — see the **Minimal in-game UI by design** bullet in `product.md`'s Race Mode section. The on-screen `Keyboard` was removed from Race Mode in this same pass, then explicitly restored shortly after (the requester wanted mouse/physical-keyboard/arrow-key input parity with Championship Mode preserved) — see the layout note below. Net effect: Race Mode's *input* chrome (grid + keyboard) is identical to Championship Mode again; only the *status/feedback* chrome (attempts counter, mid-race banners) stays Championship-only.

**Post-launch board layout pass (requested after Phase 5 shipped)**: the "Sua palavra" (own board) section's internal layout was reworked in both `ChampionshipGameRoom` and `RaceGameRoom` (`frontend/src/App.tsx`):
- The `"Sua palavra"` heading above the grid was removed entirely — the board panel is now unlabeled.
- Within each game screen's flex section, the column order is now **Outros jogadores → word-grid + keyboard panel → Placar/leaderboard** (`ScoreBoard` in Championship, `RaceLeaderboard` in Race — both referred to as "placar"), reordered twice during the pass before landing here.
- A new `.board-panel` CSS class (`frontend/src/index.css`) wraps the grid + keyboard: `align-items: flex-end` right-aligns the grid within the panel, while the on-screen `Keyboard` is stretched to `width: 100%` so it visually matches the width of the `.status-card` (`RoundStatus`/`RaceStatus`) above it — a deliberate width-matching pairing, not a coincidence of the flex layout.
- `.word-grid` gained `align-items: center` so each `.word-row` is horizontally centered within the grid's own box.
- Main structural `div`/`section`/`header`/`main` elements across `App.tsx` (and the `ScoreBoard`/`RaceLeaderboard` root, both `id="placar"`) now carry explicit, screen-prefixed `id` attributes (`championship-*`/`race-*`) for stable CSS/test selectors — only one game screen is ever mounted at a time, so the shared `id="placar"` across `ScoreBoard`/`RaceLeaderboard` never collides in practice.

Because Championship Mode and Race Mode now run side by side rather than one replacing the other (unlike the Phase 1→Phase 2 handoff), `backend/src/index.ts` starts both `ChampionshipMode` and `RaceMode` as separate `Game` instances on separate Socket.io namespaces (`/championship`, `/race`) rather than picking one as "the" live mode — a deliberate stand-in for Phase 5's real lobby/mode-selection UI. The frontend gained a minimal mode-picker screen (`frontend/src/App.tsx`) so both are reachable end-to-end; this picker is throwaway scaffolding, same spirit as Phase 1's scaffolding, and is expected to be replaced by the real lobby in Phase 5.

Two rule clarifications resolved during implementation (not spelled out in the original bullet list):
- **Per-player independent progress, not a room-wide shared word**: unlike Championship Mode, each player races through the 5-word sequence at their own pace — their own word index, own attempts, own per-word timer. This is what makes "first to finish" well-defined per `product.md`'s "race to finish detection" framing.
...
**Deliverables**: `RaceMode` implementation of `GameMode` (same interface as `ChampionshipMode` — no changes to room/use-case orchestration code, confirming the Phase 1 OCP bet paid off); shared per-word timer logic reused from Phase 2, not reimplemented.
...
P4.8: championship-mode.ts emits roomId in payloads — done

P4.9: race-mode.ts emits roomId in payloads — done
...
P4.14: rewrite register-championship-mode-handlers.ts for multi-room — done

P4.15: rewrite register-race-mode-handlers.ts for multi-room — done
...
P4.19: useGame/useRaceGame accept code param — done
...
P5.7: update create-room.ts for isPublic/settings/status lobby — done. `CreateRoomInput` now takes optional `isPublic`/`settings`; the room no longer auto-starts the game on creation — `ChampionshipMode`/`RaceMode` only get `joinPlayer`'d (lobby join), the actual round/race begins when `start-game` (P5.11) is invoked.
...
P5.10: update-room-settings.ts use-case + spec — done (5 tests). Host-only, lobby-only; pushes the validated settings into the live `ChampionshipMode`/`RaceMode` instance via a new `updateOptions()` method on each.

P5.11: start-game.ts use-case + spec — done (5 tests). Host-only, lobby-only; calls `gameMode.start()` and flips `status` to `in-progress`. Required a `ChampionshipMode`/`RaceMode` fix: since players now join during the lobby (before `start()`), `RaceMode.start()` was updated to register every already-joined player into the fresh `RaceGame` (it previously assumed `start()` ran before anyone joined).
...
P5.18: Lobby.tsx component — done, backed by a new `frontend/src/hooks/useLobby.ts` (not in the original task list, but required: it owns the lobby's socket connection — join, `lobby:state`/`game:start`/`host:migrated` listeners, and the `room:settings:update`/`room:start`/`room:leave` emitters — mirroring how `useGame`/`useRaceGame` own the in-game socket).
...
- **O — Open/Closed**: new modes (Phase 2, 3) and new room types (Phase 4, 5) are added by writing new classes (`ChampionshipMode`, `RaceMode`, public/private room policy) behind existing interfaces — Phase 1's use-cases and socket handlers are not edited to support them.
- **L — Liskov Substitution**: any `GameMode` (single-word, championship, race) must be usable wherever the use-case layer expects a `GameMode` — same lifecycle contract (`start`, `submitGuess`, `isResolved`, `getWinner`), no mode throwing on calls another mode handles.
...
- **Consistency**: domain invariants (max 6 attempts in Championship Mode, exactly 5 words per game, valid room capacity ≤ 50) are enforced in `domain` entities before any state mutation is persisted — invalid transitions are rejected, never written then cleaned up.
...
**Single-room membership (done)**: a player may only belong to one room at a time. `RoomRepository` gained a `playerId → active room code` index (`findActiveRoomCodeForPlayer`/`setActiveRoomForPlayer`/`clearActiveRoomForPlayer`, set by `create-room.ts`/`join-room.ts` and cleared by `leave-room.ts`). `join-room.ts` looks up the joining player's previous active room and, if it differs from the room being joined, internally calls `leave-room.ts` against it before completing the new join — same effect as the player explicitly leaving first. `leave-room.ts` now also explicitly deletes the room (`RoomRepository.delete`, plus tearing down its live `GameMode` from the registry and clearing its `PlayerSessionStore` entry) the moment `record.players.length === 0` after removal, instead of relying on passive Redis TTL expiry. Host migration (above) still applies first when the leaving/switching player was the host of a room that still has other players left in it. Socket handlers (`register-championship-mode-handlers.ts`/`register-race-mode-handlers.ts`) broadcast the vacated room's updated `lobby:state`/`host:migrated` and call `socket.leave()` on it — this only reaches other players when the vacated room shares the same Socket.IO namespace (mode) as the one being joined; cross-mode switches (e.g. leaving a Race room to join a Championship room) still update the repository/registry state correctly but don't push a live UI update to the now-empty room's namespace, since that namespace isn't reachable from the other handler.
...
1. **Visibility of system status**: live connection indicator, attempts-remaining counter (Championship Mode only — Race Mode hides it by design, see `product.md`), whose-turn/word-index indicator (Championship/Race Mode), real-time update of other players' boards the instant they guess.
...
4. **Consistency and standards**: one color meaning across every screen and mode; the grid + on-screen keyboard input chrome is identical between Championship Mode and Race Mode (same `.board-panel` layout, same `Keyboard` component), but the status/feedback chrome (attempts counter, mid-round reveal banners) is Championship-only by design — see Race Mode's **Minimal in-game UI** note in `product.md`.
